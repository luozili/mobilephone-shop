...
public class ProductServiceImpl implements ProductService {
	...
	public ESMssage searchProduct(String productName, int pageIndex, int size) {
		try {
			StringBuilder sb = new StringBuilder("http://193.112.173.11:8085/es/product/search");
	...
	public Message addProduct2UserCar(String pid, String token, int pnumber) {
		try {
			...
			return restTemplate.postForObject("http://193.112.173.11:8084/product/car/add", request, Message.class);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Message deleteProduct2UserCar(String cid) {
		try {
			...
			body.add("cid", cid);
			HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<>(body, header);
			return restTemplate.postForObject("http://193.112.173.11:8084/product/car/delete", request, Message.class);
		} catch (Exception e) {
			e.printStackTrace();
			return new Message(Message.fail);
		}
	...
	public Message updateProduct2UserCarPnumber(String cid, int pnumber) {
		try {
			...
			body.add("pnumber", pnumber + "");
			HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<>(body, header);
			return restTemplate.postForObject("http://193.112.173.11:8084/product/car/update/pnumber", request,
					Message.class);
		} catch (Exception e) {
			e.printStackTrace();
			return new Message(Message.fail);
	...
	public LayUITableMessage selectProduct2UserCar(String token) {
		try {
			...
			return restTemplate.getForObject("http://193.112.173.11:8084/product/car/select?uid=" + uid,
	...
	public ProductMessage selectProduct(String pid) {
		try {
			StringBuilder sb = new StringBuilder("http://193.112.173.11:8084/product/get");
	...
	public Message selectProduct2UserCarExist(String pid, String token) {
		try {
			User user = objectMapper.readValue((String) redisTemplate.opsForValue().get(token), User.class);
			StringBuilder sb = new StringBuilder("http://193.112.173.11:8084/product/car/exist");
	...
	public Message deleteProduct2UserCar(String token, String pid) {
		try {
			...
			body.add("pid", pid);
			HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<>(body, header);
			return restTemplate.postForObject("http://193.112.173.11:8084/product/car/delete/pid", request, Message.class);
		} catch (Exception e) {
			e.printStackTrace();
			return new Message(Message.fail);
		}
