...
public class UserserviceImpl implements UserService {
	...
	public User select(String username, String password) {
		try {
			...
			User user = restTemplate.postForObject("http://193.112.173.11:8082/user/login", request, User.class);
	...
	public Message insert(User user) {
		try {
			return postCU(objectMapper.writeValueAsString(user), "http://193.112.173.11:8082/user/register");
		} catch (JsonProcessingException e) {
			e.printStackTrace();
	...
	public Message update(User user, HttpServletRequest request) {
		try {
			...
			Message message = postCU(string, "http://193.112.173.11:8082/user/update");
			user.setPassword(null);
	...
	public Message insertUserAdress(Adress adress) {
		try {
			return postCU(objectMapper.writeValueAsString(adress), "http://193.112.173.11:8082/user/adress/insert");
		} catch (JsonProcessingException e) {
			e.printStackTrace();
		}
		return new Message(Message.fail);
	...
	public Message updateUserAdress(Adress adress) {
		try {
			return postCU(objectMapper.writeValueAsString(adress), "http://193.112.173.11:8082/user/adress/update");
		} catch (JsonProcessingException e) {
			e.printStackTrace();
		}
		return new Message(Message.fail);
	...
	public Message deleteUserAdress(String aid) {
		return postD("http://193.112.173.11:8082/user/adress/delete", "aid", aid);
	}

	@Override
	public LayUITableMessage selectUserAdress(String auid) {
		return getR("http://193.112.173.11:8082/user/adress/select", "?auid=" + auid, LayUITableMessage.class);
	}
