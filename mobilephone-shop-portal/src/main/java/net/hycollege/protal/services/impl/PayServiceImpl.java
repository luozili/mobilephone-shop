...
public class PayServiceImpl implements PayService {
	...
	private void addProduct2Order(String uid, String pid, int pnumber, String out_trade_no) {
		try {
			...
			restTemplate.postForObject("http://47.103.217.104:8086/order/insert", request,
					Message.class);
...
//				System.out.println("");
	...
	public Message alipayTradeRefund(String out_trade_no) {
		try {
			...
			body.add("out_trade_no", out_trade_no);
			HttpEntity<MultiValueMap<String, Object>> request = new HttpEntity<>(body, header);
			return restTemplate.postForObject("http://47.103.217.104:8086/order/refund", request, Message.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Message updateForOrder(AlipayTradePayResponse alipayTradePayResponse) {
		try {
			...
			HttpEntity<MultiValueMap<String, Object>> request = new HttpEntity<>(body, header);
			return restTemplate.postForObject("http://47.103.217.104:8086/order/update", request, Message.class);
		} catch (Exception e) {
			e.printStackTrace();
			return new Message(Message.fail);
		}
